...
output = open("RandomSearchResults.txt", "w")
output.close()
...
#All the potential hyperparamter values
alphaGrowthVals = [1.1,1.175,1.25]
betaGrowthVals = [1.1,1.175,1.25]

alphaPeakVals = [5,10,15,20]
betaPeakVals = [-5,-10,-15,-20]

boostVals = [1,3,5]

#Any particular filename can be chosen. This particular sample was randomly chosen.
fileName = '2016-05-18T10-30-00.csv'
...
for i in range(50):
	alphaG = alphaGrowthVals[int(random.random()*len(alphaGrowthVals))]
	betaG = betaGrowthVals[int(random.random()*len(betaGrowthVals))]

	alphaP = alphaPeakVals[int(random.random()*len(alphaPeakVals))]
	betaP = betaPeakVals[int(random.random()*len(betaPeakVals))]

	boost = boostVals[int(random.random()*len(boostVals))]
	
	finder = PathFinder.PathFinder(boostFactor=boost, alphaGrowth=alphaG, betaGrowth=betaG, alphaPeak=alphaP, betaPeak=betaP, numberOfAnts = 500, taxis=Taxis, passengers=Passengers, triplengths=TripLengths, search_range=5, percent_cut=100, city=scaledNYC)
	...
	result = finder.getBestValue()

	output = open("RandomSearchResults.txt", "a")
	output.write(f"{alphaG}, {alphaP}, {betaG}, {betaP}, {boost}, {result}\n")
	output.close()
